...
public class StatPrinter {
    ...
    public StatPrinter( ArrayList <Integer> data ) //O(n)
    {
	_frequency = new ArrayList <Integer>();
	while(_frequency.size() < max(data)+1) //Adds zeroes until _frequency size is one greater than the max of data
	    _frequency.add(0);
	for(int i = 0; i < data.size(); i++) { 
	    _frequency.set(data.get(i), _frequency.get(data.get(i)) + 1); //Adds one to the position in frequency corresponding to the number in data
	}
    }
    ...
    public static Integer max( ArrayList <Integer> data )  //O(n)
    { 
	int greatest = data.get(0);
	for(int i = 0; i < data.size(); i++) {
    ...
    //    isLocalMode(1) -> true
    //    isLocalMode(5) -> true
    public boolean isLocalMode( int i ) //O(n)
    {
	...
	for(int i = 0; i < _frequency.size(); i++) {
	    String s = i + " : ";
	    for(int ctr = 0; ctr < _frequency.get(i) * ratio; ctr++)
		s += "*";
