...
public class Matrix 
{
	...
	for(int row = 2; row < size() + 1; row++) {
		...
		set(col, row, set(row, col, get(col, row)) ); //ie: (2, 0) would swap with (0, 2)
	    }
	}
    }
	...
	//Testing transpose
	Matrix m3 = new Matrix(4); //4x4
	int ctr = 101;
	for(int row = 1; row < m3.size() + 1; row++) {
	    for(int col = 1; col < m3.size() + 1; col++) {
		m3.set(row, col, ctr);
		ctr+= 1;
	    }
	}
